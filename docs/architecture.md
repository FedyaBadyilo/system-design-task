...
запросу. В LLM передаются top-3 фрагмента с `article_id`, версией и score. Низкие
scores или противоречивые кандидаты означают `HUMAN_REVIEW`. Развитие — hybrid
search из BM25 и dense search на локальной предобученной embedding-модели.
