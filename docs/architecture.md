...
> **TODO → `docs/ml.md` (PII):** основной риск — false negative. Оптимизировать
> recall при контроле precision; описать разметку, пороги по типам и калибровку.

> **TODO → `docs/ml.md` (classifier):** target — итоговая операторская тема;
> временной split с группировкой связанных тикетов, веса классов и калибровка.
> Главная метрика — macro F1; дополнительно per-class recall и
> precision–coverage для принятых прогнозов.
