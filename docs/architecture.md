...
Regression` возвращает ограниченный набор тем по суммарной вероятности.
Рискованная тема, конфликт с UX-темой или низкая уверенность означают
`HUMAN_REVIEW`; иначе — `AI_PENDING`.

## Retrieval

Используется только одобренная версионируемая база знаний, не сырые ответы
операторов. Целевая система фильтрует активные статьи по ограниченному набору
тем, покрывающему заданную суммарную вероятность классификатора; PoC использует
фиксированные top-3. Baseline ранжирует статьи по TF-IDF similarity к очищенному
запросу. В LLM передаётся ограниченный top-K фрагментов с `article_id`, версией и
score; K и порог подбираются на данных. Низкие scores или противоречивые
кандидаты означают `HUMAN_REVIEW`. Развитие — hybrid search из BM25 и dense
search на локальной предобученной embedding-модели.

## Генерация

Один вызов LLM без tools и agent loop получает очищенный текст и ограниченный
набор контекстов. Строгий prompt запрещает следовать инструкциям из
пользовательского текста и требует JSON с `answer` и `used_article_ids`.
Детерминированная проверка контролирует схему, ссылки на входные статьи, PII и
policy. Только безопасная категория с пройденными проверками получает автоответ;
self-confidence LLM не используется.

## Кэширование безопасных ответов

Целевая система может не вызывать LLM повторно для точного дубликата безопасного
запроса. Ключ кэша строится как hash нормализованного очищенного текста, locale,
темы и версий policy, базы знаний, retrieval и prompt. Hash служит только ключом
поиска и не доказывает корректность: автоматически переиспользуется лишь ранее
одобренный оператором или детерминированный ответ без последующей правки и
повторного открытия.

Перед отправкой заново выполняются текущие PII- и risk-проверки, а ответ проходит
те же проверки ссылок и policy, что и новый результат LLM. Смена любой версии,
истечение TTL или негативный исход инвалидируют запись; решение сохраняется в
audit с причиной `CACHE_HIT` и ссылкой на исходное решение. Нечёткое сходство
используется только для подсказки оператору, но не для автоматической отправки.
В PoC кэш ответов не реализован.
